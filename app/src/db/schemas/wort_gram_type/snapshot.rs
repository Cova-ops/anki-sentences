use crate::db::schemas::wort_gram_type::{ModelWortGramType, SchemaWortGramType};
...
impl From<SchemaWortGramType> for SnapshotWortGramType {
    /// Don't use this on prod
    /// It doesn't handle errors
    fn from(value: SchemaWortGramType) -> Self {
        let model = ModelWortGramType::try_from(value).unwrap();
        model.into()
    }
}
...
mod tests {
    ...
    mod from_schema {
        ...
        fn happy_path() {
            ...
            let snap: SnapshotWortGramType = schema.into();
        ...
        fn panics_on_invalid_schema() {
            let invalid = SchemaWortGramType {
                id_worte: 1,
                id_gram_type: 3,
