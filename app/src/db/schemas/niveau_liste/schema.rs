...
mod tests {
    ...
    mod from_sql {
        ...
        fn ok_with_null_deleted_at() -> Result<(), DbError> {
            ...
            let mut stmt = conn.prepare("SELECT 'A2', '2025-12-04 20:00:00', NULL;")?;
            let out: SchemaNiveauListe = stmt.query_one([], SchemaNiveauListe::from_sql)?;
        ...
        fn ok_with_some_deleted_at() -> Result<(), DbError> {
            ...
            let mut stmt =
                conn.prepare("SELECT 'B1', '2025-12-04 20:00:00', '2025-12-31 00:00:00';")?;
            let out: SchemaNiveauListe = stmt.query_one([], SchemaNiveauListe::from_sql)?;
        ...
        fn err_type_mismatch() -> Result<(), DbError> {
            ...
            // 123 instead of a valid String
            let mut stmt = conn.prepare("SELECT 123, '2025-12-04 20:00:00', NULL;")?;
            let out: Result<SchemaNiveauListe, _> = stmt.query_one([], SchemaNiveauListe::from_sql);
        ...
        fn err_missing_column() -> Result<(), DbError> {
            ...
            // deleted_at field is missing
            let mut stmt = conn.prepare("SELECT 'A1', '2025-12-04 20:00:00';")?;
            let out: Result<SchemaNiveauListe, _> = stmt.query_one([], SchemaNiveauListe::from_sql);
