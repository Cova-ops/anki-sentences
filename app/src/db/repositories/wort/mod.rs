...
impl WortRepo {
    /// HashMap
    /// - key: wort_id
    /// - value: vec<SchemaWortGramType>
    fn hydrate_gram_types(
        conn: &Connection,
        worte: &[SchemaWort],
    ) -> Result<HashMap<i32, Vec<SchemaWortGramType>>, DbError> {
        if worte.is_empty() {
    ...
    pub fn bulk_insert(
        conn: &mut Connection,
        data: &[InputWort],
    ) -> Result<(Vec<SchemaWort>, HashMap<i32, Vec<SchemaWortGramType>>), DbError> {
    ...
    pub fn bulk_insert_tx(
        ...
        let mut hash_out = HashMap::<i32, Vec<SchemaWortGramType>>::new();
        for d in data {
            let params: SqlWort = d.to_owned().into();
            let raw = stmt
                .query_one(params.insert_params(), SchemaWort::from_sql)
            ...
            hash_out.insert(raw.id, raw_gram);
            vec_out.push(raw);
        }
    ...
    pub fn bulk_update_tx(
        ...
        let mut vec_out: Vec<SchemaWort> = Vec::with_capacity(data.len());
        let mut hash_out = HashMap::<i32, Vec<SchemaWortGramType>>::new();
        ...
        for (id, wort) in data {
            ...
            let raw = stmt
                .query_row(params.update_params(id), SchemaWort::from_sql)
                .map_err(DbError::with_sql(sql))?;
            ...
            WortGramTypeRepo::bulk_insert_tx(tx, &vec_gram_type)?;

            hash_out.insert(id, vec_gram_type);
            vec_out.push(raw);
        }
    ...
    pub fn fetch_by_id(
        ...
        let hash_out = Self::hydrate_gram_types(conn, &raw)?;
    ...
    pub fn fetch_by_wort(
        conn: &Connection,
        worte: &[(String, String)],
    ) -> Result<(Vec<SchemaWort>, HashMap<i32, SchemaWortGramType>), DbError> {
        if worte.is_empty() {
            return Ok(vec![]);
        }
        ...
        let mut vec_out: Vec<SchemaWort> = vec![];
        for w in worte {
            let mut stmt = conn.prepare(&sql).map_err(DbError::with_sql(sql))?;
            ...
            vec_out.append(&mut vec_raw);
        }

        let hash_out = Self::hydrate_gram_types(conn, &mut vec_out)?;
