...
impl SetzeReviewRepo {
    ...
    pub fn bulk_upsert_tx(
        ...
        let sql = r#"
            INSERT INTO setze_review (satz_id, direction, interval, ease_factor, repetitions, last_review, next_review)
                VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)

            ON CONFLICT(satz_id, direction) DO UPDATE SET 
                interval = ?3,
                ease_factor = ?4,
                repetitions = ?5,
                last_review = ?6,
            ...
            RETURNING satz_id, direction, interval, ease_factor, repetitions, last_review, next_review, created_at, deleted_at;
        "#;
    ...
    pub fn fetch_by_satz_id(
        ...
        let sql = format!(
            "
                SELECT 
                    satz_id, direction, interval, ease_factor, repetitions,
