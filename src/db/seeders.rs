...
use crate::db::{
    GramTypeRepo, NiveauWorteRepo, SchwirigkeitListeRepo,
...
pub fn init_data() -> Result<()> {
    ...
    // GenderWorte
    let data = GenderWorteRepo::bulk_insert(&SEED_GENDER_WORTE_LISTE)?;
    GenderWorteSchema::init_data(&data)?;

    // NiveauWorte
    let data = NiveauWorteRepo::bulk_insert(&SEED_NIVEAU_LISTE)?;
    NiveauWorteSchema::init_data(&data)?;

    // GramType
    let data = GramTypeRepo::bulk_insert(&SEED_GRAM_TYPE_LISTE)?;
